...
import {getMediaConditions} from '@tokens/index';
import {metaThemeDefault} from '@tokens/themes';
import {extractMetaTokenGroupValues} from '@tokens/themes/utils';
...
async function generateCssMediaCondition() {
  ...
  const cssStyles = generateMediaConditionStyles(
    (token, direction, mediaCondition) =>
      `@custom-media --b-${token}-${direction} ${mediaCondition};`,
  );
...
async function generateScssMediaCondition() {
  ...
  const scssStyles = generateMediaConditionStyles(
    (token, direction, mediaCondition) =>
      `$b-${token}-${direction}: '${mediaCondition}';`,
  );
