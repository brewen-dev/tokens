...
import type {
  ...
  MetaTokenGroupShape,
  TokenName,
} from '@tokens/themes/types';
import { metaThemePartials, metaThemeDefault } from '@tokens/themes';
import { themeNameDefault } from '@tokens/themes/constants';
import { createThemeSelector } from '@tokens/themes/utils';
import { createVarName } from '@tokens/utils';
import type { Entries } from '@tokens/types';
  ...
  return Object.entries(metaTokenGroup)
    .map((entry) => {
      ...
      return tokenName.startsWith('motion-keyframes')
        ? `${createVarName(tokenName)}:b-${tokenName};`
...
export function getKeyframes(motion: MetaTokenGroupShape) {
  return Object.entries(motion)
    ...
    .map(([tokenName, { value }]) => `@keyframes b-${tokenName}${value}`)
    .join('');
}
