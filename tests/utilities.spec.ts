...
describe('createVar', () => {
  it('converts the token into a css variable', () => {
    const token = 'foo';
    const result = createVar(token as ANY);

    expect(result).toBe(`var(--b-${token})`);
  });
});

describe('createVarName', () => {
  it('converts the token into a css variable name', () => {
    const token = 'foo';
    const result = createVarName(token as ANY);

    expect(result).toBe(`--b-${token}`);
  });
});

describe('getThemeVarNames', () => {
  it('extracts the token names', () => {
    expect(getThemeVarNames(mockTokens as ANY)).toStrictEqual([
      '--b-design-token-1',
      '--b-design-token-2',
    ]);
  });
...
describe('getMediaConditions', () => {
  it('transforms breakpoints tokens into directional media conditions', () => {
    ...
    expect(getMediaConditions(breakpoints as ANY)).toStrictEqual({
      breakpoint1: {
        // Up: sizeInPx / 16
        up: '(min-width: 1em)',
